...
func isValidSignatureConvert(c *fiber.Ctx, secret string) bool {
	filename := c.Params("filename")
	expires := c.Query("expires")
	signature := c.Query("signature")

	// ต้อง match path เดิมตอนสร้าง URL
	pathToSign := "/api/converts/" + filename
...
func GetConvertContent(c *fiber.Ctx) error {
	_ = godotenv.Load()
