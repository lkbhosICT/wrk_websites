...
import DownloadmoitBtn from "./DownloadmoitBtn"; // ตรวจสอบ path ให้ตรงกับโปรเจกต์จริง
...
const ShowPic: React.FC<Props> = ({makeBy, view, download, id , pdfname}) => {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}tasks`, {
      ...
      .then(data => {
        ...
        if(data.status == "done" || data.status == "processing" || data.status == "pending"){
        ...
        <div className="flex flex-col items-center">
          {status === "pending" && (
            <div className="flex flex-col items-center">
              ...
              <span className="mt-2 text-sm text-red-700 font-semibold">{`กรุณารอสักครู่...`}</span>
            </div>
          )}
